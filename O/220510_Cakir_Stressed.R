...
OutDir <- 'Gruffi_revision/Cakir_2019/analysis/'
dir.create(OutDir)

# load datasets ----------------------------------------------------------------

sc.data <- Read10X('Gruffi_revision/Cakir_2019/downloaded/')
colnames(sc.data)[1]
sc.data[1:10,1:10]

sc.obj <- CreateSeuratObject(counts = sc.data, project = "Cakir_2019")
...
clUMAP("predicted.cell.type", obj = sc.obj, save.plot = F)
...
pl3 <- qUMAP(feature = "Score.GO.0001935", obj = sc.obj, save.plot = F, 
             title = "endothelial cell proliferation\n(GO:0001935)",
             cols = c(alpha("grey", alpha = 0.5), "red")) + NoAxes()
pl4 <- qUMAP(feature = "nFeature_RNA", obj = sc.obj, save.plot = F, 
             cols = c(alpha("grey", alpha = 0.5), "red")) + NoAxes()
qUMAP(feature = "EOMES", obj = sc.obj, save.plot = F, 
      cols = c(alpha("grey", alpha = 0.5), "red")) + NoAxes()

png(filename = "Gruffi_revision/Cakir_2019/analysis/summary_v1.png", width = 1500,height = 1500, res = 200)
cowplot::plot_grid(pl2,pl1, pl3, pl4, ncol = 2)
dev.off()

saveRDS(sc.obj, file = "Gruffi_revision/Cakir_2019/analysis/Cakir_object_proc_220510.Rds")
# sc.obj <- readRDS(file = "Gruffi_revision/Cakir_2019/analysis/Cakir_object_proc_220510.Rds")

png("Gruffi_revision/Cakir_2019//analysis/Scores_Cakir.png", width = 1000, height = 1000)
...
pdf('Gruffi_revision/Cakir_2019/analysis/PROGENy_Cakir.pdf')
...
png('Gruffi_revision/Cakir_2019/analysis/predicted.cell.type_POSTGruffi.png')
clUMAP("predicted.cell.type.N", obj = sc.obj.subset, save.plot = F) +NoAxes() +NoLegend()
dev.off()

saveRDS(sc.obj.subset, file = "Gruffi_revision/Cakir_2019/analysis/Cakir_object_POSTGruffi_proc_220510.Rds")

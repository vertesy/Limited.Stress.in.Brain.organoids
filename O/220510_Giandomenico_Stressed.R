# Giandomenico et al stressed cells
# Oliver Eichmueller
# Tue May 10 17:45:00 2022 ------------------------------
...
OutDir <- 'Gruffi_revision/Giandomenico_2019/analysis/'
dir.create(OutDir)

# load datasets ----------------------------------------------------------------

sc.data <- read.table('Gruffi_revision/Giandomenico_2019/GSE124174_org75_seuratdata.txt', quote = "\"")
colnames(sc.data)[1]
sc.data[1:10,1:10]

sc.obj <- CreateSeuratObject(counts = sc.data, project = "Giandomenico_2020")
...
sc.obj = SetupReductionsNtoKdimensions(obj = sc.obj, nPCs = 50, dimensions=3:2, reduction="umap")

sc.obj = RunTSNE(sc.obj, dims = 1:50)

sc.obj = FindNeighbors(sc.obj, reduction = "pca", dims = 1:50)
sc.obj = FindClusters(sc.obj, resolution = c(0.1,0.2,0.3,0.5))
...
clUMAP("predicted.cell.type.N", obj = sc.obj, save.plot = F)
...
pl2 <- Seurat.utils::clUMAP(ident = "RNA_snn_res.0.1", label = T, cols = pals::tol(12), label.cex = 5,
                            obj = sc.obj, save.plot = F) +NoAxes() +NoLegend()

sc.obj <- PlotGoTermScores(obj = sc.obj, use.ensemble = T, save.UMAP = F, GO = "GO:0001935")
pl3 <- Seurat.utils::clUMAP(ident = "predicted.cell.type.N", label = T, cols = pals::tol(12), label.cex = 5,
                            obj = sc.obj, save.plot = F) +NoAxes() +NoLegend()
pl4 <- qUMAP(feature = "DDIT4", obj = sc.obj, save.plot = F,
             cols = c(alpha("grey", alpha = 0.5), "red")) + NoAxes()


png(filename = "Gruffi_revision/Giandomenico_2019/analysis/summary_v1.png", width = 1500,height = 1500, res = 200)
cowplot::plot_grid(pl2,pl1, pl3, pl4, ncol = 2)
dev.off()

saveRDS(sc.obj, file = "Gruffi_revision/Giandomenico_2019/analysis/Giandomenico_object_proc_220510.Rds")
# sc.obj <- readRDS(file = "Gruffi_revision/Giandomenico_2019/analysis/Giandomenico_object_proc_220510.Rds")

png("Gruffi_revision/Giandomenico_2019/analysis/Scores_Giandomenico.png", width = 1000, height = 1000)
...
sc.obj@meta.data %>%
  filter(nFeature_RNA <1000) %>% count(predicted.cell.type.N)

VlnPlot(sc.obj, features = c("nFeature_RNA", "nCount_RNA", "percent.mito"), pt.size = 0, group.by = "predicted.cell.type.N")

Seurat.utils::clUMAP(ident = "is.Stressed", label = T, cols = pals::tol(12), label.cex = 5,
                     obj = sc.obj, save.plot = F, reduction = "tsne") +NoAxes() +NoLegend()

FeaturePlot(features = c("L1CAM", "NRN1", "NRP1", "UNC5D"), object = sc.obj,
      cols = c(alpha("grey", alpha = 0.5), "red")) & NoAxes()
...
pdf('Gruffi_revision/Giandomenico_2019/analysis/PROGENy_Giandomenico.pdf')
...
png('Gruffi_revision/Giandomenico_2019/analysis/predicted.cell.type_POSTGruffi.png')
clUMAP("predicted.cell.type.N", obj = sc.obj.subset, save.plot = F) +NoAxes() +NoLegend()
dev.off()

saveRDS(sc.obj.subset, file = "Gruffi_revision/Giandomenico_2019/analysis/Giandomenico_object_POSTGruffi_proc_220510.Rds")
# sc.obj.subset <- readRDS(file = "Gruffi_revision/Giandomenico_2019/analysis/Giandomenico_object_POSTGruffi_proc_220510.Rds")
